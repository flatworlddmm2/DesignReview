...
1. Optionally commit a starter `poll-data.json` in the repo root (the empty `{ "poll": null, "responses": [] }` is enough). If you skip this, the file is created automatically the first time you publish a poll from the page.
...
- To run a new poll, archive the current one, then build and publish the next from the page. Votes for a replaced poll are rejected automatically.

## Publishing polls from the page

With the relay enabled, the setup screen's button becomes "Publish poll": after a quick confirmation, the poll (designs, links and screenshots included) is written straight into `poll-data.json` in the repo, and every visitor sees it immediately. No downloading, renaming or committing is needed. Publishing replaces the live poll and clears its reviews, so archive the current poll first if you want to keep them. Without the relay, the button stays "Launch poll" and only affects your own browser.
...
When a poll is finished, use "Archive this poll" in the Data file card on the Results tab (relay setup and your `ADMIN_KEY` required, same as flushing). Archiving:

- snapshots the poll and every review into `archive/<date>-<title>-<id>.json` in the repo,
- clears the live poll (`poll-data.json` becomes empty and `results.csv` is reset), so the site shows the setup screen ready for the next poll, and old-poll votes are rejected,
- keeps every archived poll browsable in the page itself: "View archived polls" appears on the setup screen (and "View archives" on the Results tab). Archived polls open read-only with their ranking, scores and comments intact.

The archive files are ordinary data files, so any of them can also be re-imported through "Import data file" if you ever want to bring an old poll back, and they remain in the repo's history like everything else. To store archives in a different folder, set an `ARCHIVE_DIR` variable on the worker.
